...
run_test()
{
	...
	make_jail $jail_home bin/pear_map $run_input
	timeout -k 5 5 firejail --profile=jail.profile --quiet --private=$jail_home ./pear_map `basename $run_input` 2>&1 | grep -v "Reading profile" > "$run_output"
	return $?
}
