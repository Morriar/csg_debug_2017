...
run_test()
{
	...
	timeout -k 5 5 firejail --profile=jail.profile --quiet --private=$jail_home ./doors.sh "$run_input" 2>&1 | grep -v "Reading profile" > "$run_output"
...
run_all()
{
	...
	for input in $(ls -d "$tests_dir"/*.in); do
		ALL=$(($ALL + 1))
